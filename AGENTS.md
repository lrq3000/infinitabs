...
### Prime directive
- Your job is to deliver PR branches that never lose already-merged changes from `main`.

### Allowed Git operations on PR branches
- ✅ You MAY add commits (including merge-resolution commits).
- ✅ You MAY merge `origin/main` into your PR branch.
...
3) For each conflicted file:
   ...
   - Adapt only when required by `main` refactors/contracts, but preserve the intent of each hunk.
   - **Strict hunk checklist:** for every hunk in `/tmp/pr.patch`, record exactly one:
     - `Applied`
     - `Applied with adaptation` (1-sentence rationale)
     - `Not applicable` only if `main` already includes the same change (1-sentence rationale)
   - Stage:
     - `git add <file>`
