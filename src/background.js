...
async function handleUnmountLogicalTab(windowId, logicalId) {
    ...
    if (logical.liveTabIds.length > 0) {
        ...
        // Prevent closing the window if these are the last tabs
        try {
            const tabs = await chrome.tabs.query({ windowId });
            const allIds = new Set(tabs.map(t => t.id));
            const remainingCount = tabs.length - toClose.filter(id => allIds.has(id)).length;

            if (remainingCount === 0) {
                await chrome.tabs.create({ windowId, url: "about:blank" });
            }
        } catch (e) {
            console.warn("Failed to check remaining tabs", e);
        }
