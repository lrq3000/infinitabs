...
async function handleMoveLogicalTabs(windowId, logicalIds, targetLogicalId, position) {
    ...
    if (firstMovedIndex !== -1) {
        ...
        const anchorLogical = findClosestAboveLiveNeighbor(reloadedSession.logicalTabs, firstMovedIndex);
        if (anchorLogical) {
            try {
                // Use the last live tab of the logical tab (if multiple, rare but possible)
                const lastLiveTabId = anchorLogical.liveTabIds[anchorLogical.liveTabIds.length - 1];
